...
class CBOWDataset():
    ...
    def get_vocab(self):
        ...
        self.vocab_freq = dict(sorted(self.vocab_freq.items(), key=lambda k: k[1], reverse=True))
        self.vocab_freq = dict(itertools.islice(self.vocab_freq.items(), self.num_vocab))
...
def create_dataloader(left_cxt, right_cxt, left_lbl, right_lbl, val_split, batch_size, seed):
    ...
    train_left_ds = TensorDataset(torch.Tensor(train_left_cxt), torch.Tensor(train_left_lbl))
    train_left_loader = DataLoader(train_left_ds, batch_size=batch_size, shuffle=True, drop_last=True, num_workers=1, pin_memory=True)
    train_right_ds = TensorDataset(torch.Tensor(train_right_cxt), torch.Tensor(train_right_lbl))
    train_right_loader = DataLoader(train_right_ds, batch_size=batch_size, shuffle=True, drop_last=True, num_workers=1, pin_memory=True)

    val_left_ds = TensorDataset(torch.Tensor(val_left_cxt), torch.Tensor(val_left_lbl))
    val_left_loader = DataLoader(val_left_ds, batch_size=batch_size, shuffle=False, drop_last=True, num_workers=1, pin_memory=True)
    val_right_ds = TensorDataset(torch.Tensor(val_right_cxt), torch.Tensor(val_right_lbl))
    val_right_loader = DataLoader(val_right_ds, batch_size=batch_size, shuffle=False, drop_last=True, num_workers=1, pin_memory=True)
    logger.info("Created Training and Validation Data Loaders for Left and Right Branches")
